...
class TestRole(TestCase):
    ...
    def test_has_permission_immediate_no_params(self):
        ...
        self.assertTrue(
            subrole.instantiate({}).has_privilege(superrole1.instantiate({}))
        )
        ...
        self.assertFalse(
            subrole.instantiate({}).has_privilege(superrole2.instantiate({}))
        )
    ...
    def test_has_permission_immediate_params(self):
        ...
        arbitrary.grant(
            to_role=superrole1,
            from_role=subrole,
            assignment=dict(one='foo')
        )

        self.assertTrue(
            subrole.instantiate({}).has_privilege(
                superrole1.instantiate(dict(one='foo'))))
        self.assertFalse(
            subrole.instantiate({}).has_privilege(
                superrole1.instantiate(dict(one='baz'))))
...
class TestGrant(TestCase):
    ...
    def test_instantiated_to_role_smoke_test(self):
        ...
        self.assertEqual(
            grant.instantiated_to_role({}).assignment,
            {'one': 'hello'},
        )

        grant = arbitrary.grant(
            to_role=superrole,
            assignment={'two': 'goodbye'},
        )
